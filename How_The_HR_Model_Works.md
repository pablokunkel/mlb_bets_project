...
A plain-English walkthrough of how the model decides who's most likely to hit a home run on any given day. Last updated 2026-08-21.

Last verified: 2026-08-21 against commit `eb76a82` — weights, anchors, and selection rules below were checked line-by-line against the live code (audit 2026-08-21, finding P2-7).
...
Every batter in every MLB game today gets a score from 0 to 100 called the **composite**. The composite is a weighted blend of six factors: how much raw power the batter has, how bad a matchup the pitcher is for him, how friendly the ballpark is to his handedness, how hot he's been lately, what the weather looks like at first pitch, and where he hits in the lineup. (Since the A1 weight refit of 2026-06-02, the lineup factor carries zero weight — it's still scored and shown on the board, but it doesn't move the composite.)

We sort the full board by composite, then take the top 8 — but with a few guardrails: no more than 2 picks from any single game (so we're not over-stacked on one matchup), only confirmed starters with a real batting-order spot 1-9 (no bench guys), no duplicate names, and nobody flagged as likely out (IL/scratch). Tiers (T1/T2/T3 by season HR rate) appear as labels on the board but **don't gate selection** — the picks are just the top 8 composite scores subject to those guardrails.
...
All of this is pulled by the morning ETL once and cached so the 09:07 ET scoring run (GitHub Actions cron `7 13 * * *` = 13:07 UTC, an off-peak minute chosen to reduce scheduled-cron drift) finishes in a few minutes (cold cache the first time can be 30-50 minutes due to the per-pitcher Statcast pulls; cached subsequent runs are quick).
...
### Factor 1 — Power Score (weight: 0.48 — the heaviest factor since the A1 refit)
...
Each input is scaled to 0-100 against fixed anchors, and the available ones are averaged. The current anchors (post-B17 recalibration, 2026-05-27) are set to the empirical p10 → 0 / p90 → 100 of the actual scored board:
...
| HR/FB %           | 3       | 10        |
| ISO               | 0.100   | 0.250     |
| xwOBA on contact  | 0.260   | 0.390     |
| pull-FB %         | 8       | 22        |

Anchor history: the 2026-05-03 re-tune (PR #25) first tightened the original generous ranges so MLB-leading values actually reached the top of the scale (Aaron Judge's 17% barrel was scoring 68; after PR #25 it was 100). Then the 2026-05-27 recalibration (B17) recognized that the scored board (T1-T4 + bench) is a much broader population than "MLB regulars," so the PR #25 anchors sat too high — the old HR/FB 8-20 range was clamping ~75% of the board to score 0. B17 re-anchored against the empirical p10/p90 of the 2025 backfill sample: barrel 5-15 → 3-11, HR/FB 8-20 → 3-10, ISO 0.130-0.300 → 0.100-0.250, xwOBA 0.330-0.450 → 0.260-0.390. Exit velo (85-95) and pull-FB (8-22) were left unchanged. See WEIGHT_REFIT_LOG.md 2026-05-27 for the raw percentiles.
...
### Factor 2 — Matchup Score (weight: 0.28 — second only to power)
...
- **No platoon bonus (v2 only).** Earlier versions added a flat bonus for opposite handedness, but that was double-counted with the handedness already baked into archetype similarity (handedness carries 0.20 weight inside the similarity blend). v2 — the default path — adds no platoon bonus.

If archetype data is missing (new pitcher, no HR history for the batter, or it's a slate where per-player Statcast isn't running), the model falls back to **v1 matchup scoring**: an equal-weight mean of whichever signals are available (pitcher vulnerability, woba_vs_hand, Vegas implied team total — missing ones simply drop out of the mean), PLUS a flat +10 platoon bonus when the batter has opposite handedness to the pitcher (result capped at 100). The asymmetry is deliberate: v1 has no archetype similarity to fold handedness into, so it keeps the explicit bonus that v2 removed.

### Factor 3 — Park Score (weight: 0.04)
...
**Park's weight history — zero, then additive workaround, now an ordinary weighted factor.** When the model was retrained on enriched data with logistic regression (March 2026), park factor came out essentially non-predictive *net of the other features* — the signal was already captured indirectly through pitcher vulnerability (pitchers in homer-friendly parks have inflated HR/9) and weather (warm parks tend to be hitter-friendly) — so the weight went to zero. But park-as-within-slate-percentile *is* a real edge (Yankee Stadium at the top of today's slate vs. Petco at the bottom is a 25-point spread a zero weight throws away), so PR #25 (2026-05-03) added a `+0.05 × park_score` additive bonus on top of the weighted average as a workaround.

**The A1 refit (2026-06-02) removed that additive bonus.** Park is now an ordinary weighted factor at 0.04 inside the weighted mean — the within-slate park signal lives in the weighted average like every other factor, and there is no separate additive term. (Park counted twice — weight plus additive — was exactly the redundancy A1 closed; don't re-add it.) In practice park contributes 0-4 points to the composite. The dashboard still surfaces park as its own column for diagnostic context.

### Factor 4 — Form Score (weight: 0.12)

"Is this batter hot right now?"

Three inputs drawn from MLB Stats API game logs over split game-count windows (rebuilt 2026-05-19, PR #56; AVG term dropped 2026-05-26 under B11):
...
| `ev_trend`         | nightly Statcast | -3.0 → +3.0 mph     | Real exit-velocity trend vs. season. **Currently always NULL by design — gated on the A2 nightly Statcast ETL — so it's skipped.** |

**B11 (2026-05-26) dropped `recent_avg_30g`.** `backtest_form_anchors.py` showed on three sample sizes that the AVG term is net-noise for HR prediction — dropping it lifts AUC +0.018 consistently. AVG mostly captures singles and groundballs falling in; ISO already covers the power dimension; and feast-or-famine power hitters have lower AVG by definition, so the term anti-correlated with the HR signal we want. The column stays in `pick_inputs` so backtest replay of pre-B11 dates remains possible.

Each populated input is scaled to 0-100 against its anchor; available inputs are equally-weight averaged. A None input is SKIPPED (no data); a real 0 is scored honestly. Score is the mean of however many inputs were measured; falls back to 50 if all three are NULL.
...
Weight history: the v1 regression refit pushed form's weight from ~0.15 to ~0.28, but the A1 refit (2026-06-02, fit against the 2025-season backfill) brought it back down to 0.12 — once the power anchors were recalibrated (B17), power absorbed most of the weight form had been carrying.
...
### Factor 5 — Weather Score (weight: 0.08 — small but real)
...
### Factor 6 — Lineup Score (weight: 0.00 since the A1 refit)
...
This factor wasn't in earlier versions and was added specifically because expected-PAs is a real HR signal we were ignoring. The A1 refit (2026-06-02), however, fit lineup's weight to 0.00 — the factor is still computed and displayed on the board, but it currently contributes nothing to the composite.

---

## Step 4: Combine into the composite

The composite is a weighted average of the six sub-scores times a platoon multiplier:
...
The old `+0.05 × park` additive bonus (PR #25) was **removed** in the A1 refit — park now sits inside the weighted average at 0.04 (see Factor 3).

The v1/v2 weights were originally learned via logistic regression on a 20-day enriched backfill (~5,200 batter-game rows, hit_hr as target, 7 features standardized). The current weights come from the **A1 refit (shipped 2026-06-02)**, fit against the much larger 2025-season backfill (`pick_inputs` joined to outcomes) — a power-led blend chosen as a judgment call over the raw FREE candidate's form≈0 / lineup-0 concentration; see WEIGHT_REFIT_LOG.md 2026-06-02 for the FREE/PINNED out-of-sample analysis. Pre-A1 backtest progression, for history:

- **Legacy fixed-anchor model:** 36.04% top-8 hit rate
- **v1 (learned weights + within-slate percentile rerank):** 38.75%
- **v2 (learned weights + xwoba_contact + fb_pct_allowed + Vegas):** 40.00%

> **About that 40%:** that figure came from an in-sample backtest. Live performance was running ~36-40% through April 2026 — and then dropped to ~17% in early May before we discovered (2026-05-04) that lineups were being read from MLB's bdfed/matchup endpoint, which returns the alphabetical 26-man roster, not the batting order. The model was effectively scoring random hitters at "position 1-9" for any team without a posted lineup — which is most teams at scoring time (then noon ET; the run now fires even earlier, at 09:07 ET, which is why the recent-lineup fallback is the normal path today). After the lineup endpoint fix (PR #32) and recent-lineup fallback (PR #33), live performance recovered to 37.5% on the first two clean days (5/4 + 5/5: 6 of 16). We're treating the 36-40% backtest band as the realistic target, not a guarantee.
...
```
weighted_avg = 0.48×82 + 0.28×70 + 0.04×95 + 0.12×60 + 0.08×55 + 0.00×80
             = 74.36
composite    = 74.36 × 1.0 = 74.4
```

That'd put him near the top of the board — likely a pick. If Harper were a 75% platoon bat instead, the dampener (0.975) would knock him to ~72.5.

---

## Step 5: Build the 8-pick card

After scoring, we have a "full board" of every scored batter sorted by composite. Selection has seven rules, applied in order:

1. **Take the highest composite score available.**
2. **No duplicate names.** The selection loop tracks every name already on the card and skips repeats — the same player can appear on the full board more than once (e.g., scored in a tier pool and again in the T4-untiered pass), but only makes the card once.
3. **Max 2 picks per game (game_pk).** If a single game's lineup has 4 batters in the top 8 by raw composite, only the top 2 of those make the card. The rest stay on the visible board but don't get a star. This prevents one rained-out game from torching half the card.
4. **Must be a confirmed starter.** `batting_order` integer between 1 and 9. Anyone with batting_order = NULL, "bench", or 10+ is excluded. (This was a real bug fix — before the filter was tightened, the model occasionally picked Seiya Suzuki at bench position 11.)
5. **Must not be flagged likely-out (B7 IL/scratch filter, added 2026-05-25).** Anyone with `is_likely_out` (status code other than Active — IL, paternity, scratch) is skipped for selection. The row stays on the full board for diagnostic visibility (composite preserved, IL badge rendered) but gets `selected=0`; the rank-9+ player promoted into the freed slot is tagged `promoted_due_to='il_filter'` for calibration audits.
6. **Game must not be postponed/cancelled/suspended (added 2026-05-05, PR #40).** `generate_picks` filters games out of the slate before scoring whenever `gameStatus.detailedState` matches `Postponed*`, `Cancelled*`, or `Suspended*` (substring + case-insensitive). Originating case: 2026-05-05 NYM @ COL was scheduled and posted lineups, then got rained out and rescheduled — but the lineup data was still in our DB at scoring time. Without this filter, generate_picks would happily score that game's batters and the card could include picks for a game that won't be played.
7. **Pick-input row hygiene** (added 2026-05-01): `load_picks_to_db.py` excludes from `pick_inputs` any row where all four power inputs (barrel%, exit velo, HR/FB%, ISO) are missing or zero. These rows would otherwise poison the per-factor decomposition charts and the weight refit's training data with all-zero feature vectors.

**No-games days and the simulator (B33, PR #118).** Since B33, a day with no eligible games publishes a zero-pick "no games" payload (`mode=no_games`) and a production run never simulates — the offline simulator is reachable only via `--offline`, and a schedule/API failure fails loud (non-zero exit, nothing published) instead of inventing a slate.
...
- **Park's weight is small (0.04) and its signal is mostly indirect.** The A1 refit brought park back into the weighted average (replacing the PR #25 additive-bonus workaround), but pitcher vulnerability still captures most of "homer park" through inflated HR/9, so park's direct weight stays modest.
- **Per-pitcher Statcast on live daily runs.** Cold pitcher arsenal cache on the daily path can still take a few minutes for new starters (rookies, fresh callups). The historical-reconstruction path (backfill, backtest) is fixed as of 2026-05-22 — `pitcher_profile._build_victim_profiles_from_db` reads everything from local SQLite. The live daily path's cold-callup case is the remaining cost; mitigated by overnight cache pre-warming.
...
For every batter in today's lineup, we score six things on a 0-100 scale: raw power against empirically-anchored distributions with a season-HR floor (48%), pitcher matchup including vulnerability + archetype-similarity-from-HR-history + Vegas implied total + a +15 rookie-pitcher bonus (28%), park HR factor (4%), recent form over 10/30-game windows (12%), weather including handedness-aware wind direction (8%), and batting-order position (scored and displayed, but zero-weighted since the A1 refit). The composite is multiplied by a [0.90, 1.0] platoon dampener that knocks part-time bats down a few ranks. We sort the full board by composite, take the top 8 with no more than 2 picks per game, no duplicate names, only confirmed starters at batting positions 1-9 who aren't flagged likely-out (IL/scratch), and only games that aren't postponed/cancelled/suspended. Tier labels (T1/T2/T3 + T4-Untiered for low-game starters) appear on the dashboard but don't gate selection. After the day finishes and outcomes come in, a battery of diagnostics — input calibration curves, temp×humidity heatmaps, elite-pitcher-archetype dampening checks, rank-band HR hitter splits — feeds the next round of tuning. That's the model.
