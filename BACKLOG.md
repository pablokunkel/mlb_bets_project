...
- **Tested 2026-09-11, NOT shipped — multi-input `model_prob`.** Logistic on 21 stored inputs (composite, factor scores, season + prior-season HR, ISO, xwOBA, pitcher HR/9 + K/9, park, temp, dome, Vegas total, batting order…) vs composite-only Platt, three time splits on the 19,552 live starters: AUC +0.010 (0.593 → 0.603), Brier −0.0002, but the **top 3% is overconfident** — predicts 24-29% where the actual is 14-16%, while Platt predicts 19-20% vs 16-20% actual. Since the edge card lives entirely in that tail, the composite-only calibration stays. Strongest standardized coefficients for the record: prior-season HR +0.21, Vegas team total +0.12, ISO +0.11, batting order −0.13. Next attempt should be isotonic / binned calibration of a richer score, not a raw logistic.
- **Open (step 5 + follow-ups):** decide "most likely" vs "+EV" after ≥30 priced days; revisit the 1.07 overround once any two-way rows exist; retry `model_prob` with isotonic calibration when there are ~60 priced days to validate the tail.
...
- **2026-09-12:** **PR #141** noon odds capture `--top 60` (near-full slate, ~24 credits/day vs 422 left for ~16 days); **PR #142 (B40c)** edge card requires book prob ≥ 10% (first board-priced day had produced eight +1200-2000 long-shots — calibration tail, not edge) + `edge_by_factor` residual table (actual − book by factor tercile) on the Performance tab. A desktop scheduled task `mlb-hr-daily-health-check` (09:05 local, daily) now verifies the five workflows, the Cloudflare build, the live slate date, odds credits, smoke, and the market-edge numbers, and fixes breakages with a single PR off main; Pablo delegated the season to Claude on 2026-09-12.
